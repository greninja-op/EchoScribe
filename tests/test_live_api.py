...
class TestLiveEchoScribeAPI(unittest.TestCase):
    ...
    def test_milestones_and_10k_simulation(self):
        ...
        resp = self.client.get("/api/milestones")
        self.assertEqual(resp.status_code, 200)
        data = resp.json()
        ...
        resp2 = self.client.post("/api/milestones/simulate-10k", json={"total_words": 10500})
    ...
    def test_dictionary_and_tones(self):
        ...
        resp = self.client.post("/api/dictionary/apply", json=payload)
        self.assertEqual(resp.status_code, 200)
        res = resp.json()
    ...
    def test_transcribe_endpoint(self):
        ...
        resp = self.client.post("/api/transcribe", files=files, data=data)
        self.assertEqual(resp.status_code, 200)
        res = resp.json()
